...
RUN pip install --user virtualenv \
    && /home/oioioi/.local/bin/virtualenv -p python3.8 venv

COPY --chown=oioioi:oioioi setup.py /sio2/sioworkers/
...
COPY --chown=oioioi:oioioi config/supervisord.conf /sio2/sioworkers/config/supervisord.conf
